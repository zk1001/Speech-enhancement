...
fig = findall(0,'type','figure','tag','Fig4','name','Model Reference Control');
...
elseif (strcmp(cmd,'training_con') | strcmp(cmd,'cont_training_con') | strcmp(cmd,'data_ok') | ...
  ...
  if (strcmp(cmd,'gen_data') | strcmp(cmd,'have_file'))%strcmp(cmd,'start_training')
    ...
    if strcmp(cmd,'have_file')
     ...
     'String','Accept Data', ...
      ...
      if (length(a1) == 0 | length(a2) == 0), 
         ...
         ref_path=a2(1:strfind(a2,a1)-1); set(H.Ref_file_ptr,'userdata',Ref_file);  
      end
    ...
    set(fig,'userdata',H)
    set(fig2,'UserData',f2); clear fig H fig2 f2
    vars = allDoubleLogicalAndStruct(whos);
    save(cat(2,tempdir,'ind_adap_data2.mat'),vars{:});
    return;
    
  elseif strcmp(cmd,'data_ok')
    load(cat(2,tempdir,'ind_adap_data2.mat'));
    delete(cat(2,tempdir,'ind_adap_data2.mat'));
    fig2=findall(0,'type','figure','tag','ind_adap_data'); delete(fig2);
    ...
    clear fig H
    vars = allDoubleLogicalAndStruct(whos);
    save(cat(2,tempdir,'nnmodrefdata.mat'),vars{:});
    return
  ...
  elseif strcmp(cmd,'training_con') 
    ...
    IW=eval(nnstring.vert_cat(get_param(arg1,'IW')));
    LW2_1=eval(nnstring.vert_cat(get_param(arg1,'LW2_1')));
    LW1_2=eval(nnstring.vert_cat(get_param(arg1,'LW1_2')));
    B1=eval(nnstring.vert_cat(get_param(arg1,'B1')));
    B2=eval(nnstring.vert_cat(get_param(arg1,'B2')));
...
%     netn = newff(mM,[S1c S2 S1 S2],{tf1,tf2,tf1,tf2},'trainlm');
    netn = feedforwardnet([S1c S2 S1]);
    netn.layers{2}.transferFcn = tf2;
    netn = configure(netn,mM);
%     warning(ws)
    ...
    if Use_Previous_Weights & ~isempty(nnstring.vert_cat(get_param(arg1,'IW_y')))
      ...
      IW_y2 = eval(nnstring.vert_cat(get_param(arg1,'IW_y')));
      IW_r2 = eval(nnstring.vert_cat(get_param(arg1,'IW_r')));
      IW_u2 = eval(nnstring.vert_cat(get_param(arg1,'IW_u')));
      LW_c2 = eval(nnstring.vert_cat(get_param(arg1,'LW_c')));
      B1_c2 = eval(nnstring.vert_cat(get_param(arg1,'B1_c')));
      B2_c2 = eval(nnstring.vert_cat(get_param(arg1,'B2_c')));
...
%     netn = newff([mint maxt],[S1c S2 S1 S2],{tf1,tf2,tf1,tf2},'trainbfgc');
    netn = feedforwardnet([S1c S2 S1]);
    netn.inputs{1}.processFcns = netn.inputs{1}.processFcns(2);
    netn.layers{2}.transferFcn = tf2;
%     warning(ws)
...
%     netn.trainParam.show=1;
    ...
    netn = configure(netn,con2seq(tr_dat.U'),con2seq(tr_dat.Y'));
    ...
    netn.layers{1}.name = 'Controller Hidden';
    netn.layers{2}.name = 'Controller Output';
    netn.layers{3}.name = 'Plant Hidden';
    netn.layers{4}.name = 'Plant Output';
    netn.inputs{1}.name = 'Reference';
    netn.name = 'Model Reference Control';
  end
...
%            [netn,tr,Yout,E,Pf,Af,flag_stop] = trainbfgc(netn,rrxx,yyxx,ui,[],epochs_c,Partition,par_size);
            [netn,tr] = train(netn,rrxx,yyxx,ui,[]);
            [YYY,XXXf,Af] = netn(rrxx,ui,[]);
        else
           Ai=Af;
...
%            [netn,tr,Yout,E,Pf,Af,flag_stop] = trainbfgc(netn,rrxx,yyxx,ui,Ai,epochs_c,Partition,par_size);
           [netn,tr] = train(netn,rrxx,yyxx,ui,Ai);
           [YYY,XXXf,Af] = netn(rrxx,ui,Ai);
        end
        flag_stop = 0;
        if flag_stop
           break
...
%         [netn,tr,Yout,E,Pf,Af,flag_stop] = trainbfgc(netn,rrxx,yyxx,ui,[],epochs_c,Partition,par_size);
  ...
  set(get(f2.h2,'XLabel'),'string','time (s)');
  set(fig,'UserData',H);
  set(fig2,'UserData',f2); clear fig fig2 H f2
  vars = allDoubleLogicalAndStruct(whos);
  save(cat(2,tempdir,'nnmodrefdata.mat'),vars{:});
  fig2=findall(0,'type','figure','tag','ind_adap_data'); 
  f2 = get(fig2,'userdata');
  fig = findall(0,'type','figure','name','Model Reference Control','tag','Fig4'); H = get(fig,'userdata');
...
H.sam_training_c_edit = uicontrol('Parent',fig, ...
 ...
 H.Handles.Menus.Help.Simulation = uimenu('Parent',H.Handles.Menus.Help.Top, ...
  ...
  S1c= convertEmptyBrackets(get_param(arg1,'S1c'));                % S1c is ASCII
  ...
  Ref_file=get_param(arg1,'Ref_file'); 
  if isempty(Ref_file)        % If the field is empty we initialize default value.
     Ref_file='';
  end
  ...
  Nrc=convertEmptyBrackets(get_param(arg1,'Nrc')); 
  ...
  Nic=convertEmptyBrackets(get_param(arg1,'Nic')); 
  ...
  Njc=convertEmptyBrackets(get_param(arg1,'Njc')); 
  ...
  Use_Inc_training=convertEmptyBrackets(get_param(arg1,'Use_Inc_training')); 
  ...
  sam_training_c=convertEmptyBrackets(get_param(arg1,'sam_training_c')); 
  ...
  epochs_c=convertEmptyBrackets(get_param(arg1,'epochs_c')); 
  ...
  retraining_c=convertEmptyBrackets(get_param(arg1,'retraining_c')); 
  ...
  max_r=convertEmptyBrackets(get_param(arg1,'max_r')); 
  ...
  min_r=convertEmptyBrackets(get_param(arg1,'min_r')); 
  ...
  max_r_int=convertEmptyBrackets(get_param(arg1,'max_r_int')); 
  ...
  min_r_int=convertEmptyBrackets(get_param(arg1,'min_r_int')); 
  ...
  Normalize=str2num(convertEmptyBrackets(get_param(arg1,'Normalize'))); 
...
errordlg(theMessage,'Model Reference Control Warning','modal');
set(fig,'pointer','arrow');
...
function names = allDoubleLogicalAndStruct(v)

names = {v.name};
classes = {v.class};
for i=numel(names):-1:1
  c = classes{i};
  if ~strcmp(c,'double') && ~strcmp(c,'struct') && ~strcmp(c,'logical')
    names(i) = [];
  end
end