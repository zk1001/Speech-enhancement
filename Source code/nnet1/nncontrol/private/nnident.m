...
H.Handles.Menus.Help.PlantIdent = uimenu('Parent',H.Handles.Menus.Help.Top, ...
  ...
  S1=convertEmptyBrackets(get_param(arg1,'S1'));                % S1 is ASCII
  ...
  sim_file=convertEmptyBrackets(get_param(arg1,'sim_file')); 
  ...
  Ts=convertEmptyBrackets(get_param(arg1,'Ts')); 
  ...
  Ni=convertEmptyBrackets(get_param(arg1,'Ni')); 
  ...
  Nj=convertEmptyBrackets(get_param(arg1,'Nj')); 
  ...
  vv=nnstring.first_match(trainfun,func_index);
  set(H.trainfun_edit,'value',vv);
  
  Use_Validation=convertEmptyBrackets(get_param(arg1,'Use_Validation')); 
  ...
  Use_Testing=convertEmptyBrackets(get_param(arg1,'Use_Testing')); 
  ...
  max_i=convertEmptyBrackets(get_param(arg1,'max_i')); 
  ...
  min_i=convertEmptyBrackets(get_param(arg1,'min_i')); 
  ...
  max_i_int=convertEmptyBrackets(get_param(arg1,'max_i_int')); 
  ...
  min_i_int=convertEmptyBrackets(get_param(arg1,'min_i_int')); 
  ...
  Limit_output=convertEmptyBrackets(get_param(arg1,'limit_output')); 
  if isempty(Limit_output)
     Limit_output='0';
  end
  set(H.Limit_output_data,'value',str2num(Limit_output))
  
  max_out=convertEmptyBrackets(get_param(arg1,'max_output')); 
  ...
  min_out=convertEmptyBrackets(get_param(arg1,'min_output')); 
  ...
  Use_Previous_Weights=convertEmptyBrackets(get_param(arg1,'Use_Previous_Weights')); 
  ...
  sam_training=convertEmptyBrackets(get_param(arg1,'sam_training')); 
  ...
  epochs=convertEmptyBrackets(get_param(arg1,'epochs')); 
...
elseif strcmp(cmd,'limit_output') && ~isempty(fig)
...
elseif strcmp(cmd,'data_no_ok') && ~isempty(fig)
...
elseif strcmp(cmd,'erase_data') && ~isempty(fig)
  set(H.Max_input,'enable','on')
  set(H.Max_input_text,'enable','on')
  set(H.Min_input,'enable','on')
  set(H.Min_input_text,'enable','on')
...
elseif (strcmp(cmd,'start_training') | strcmp(cmd,'continue_training') | strcmp(cmd,'data_ok') | ...
      strcmp(cmd,'gen_data') | strcmp(cmd,'have_file')) && ~isempty(fig)
  if strcmp(cmd,'gen_data') && ~isempty(fig)
%    H.Data_Generated=1;
    H.Data_Imported=0;
%    H.Training_done=0;
    set(fig,'UserData',H);

  elseif strcmp(cmd,'have_file') && ~isempty(fig)
    ImportStr=arg1;
    H.Data_Imported=1;
    if nargin==3
       Data_Name=arg2;
  ...
  if (strcmp(cmd,'gen_data') | strcmp(cmd,'have_file'))%strcmp(cmd,'start_training')
    ...
    if strcmp(cmd,'have_file')
     ...
     'String','Accept Data', ...
      ...
      if isempty(LastPath),
         ...
         if isempty(a2), a2 = which(cat(2,a1,'.slx')); end
      else
         a2 = which(cat(2,LastPath,cat(2,a1,'.mdl')));
         if isempty(a2), a2 = which(cat(2,LastPath,cat(2,a1,'.slx'))); end
      end
      if (length(a1) == 0 | length(a2) == 0), 
        ...
        sim_path=a2(1:strfind(a2,a1)-1); set(H.sim_file_ptr,'userdata',sim_file);  
      end
    ...
    set(fig,'userdata',H), set(fig2,'UserData',f2);
    clear fig fig2 H f2
  ...
  elseif strcmp(cmd,'data_ok')
    ...
    delete(cat(2,tempdir,'nnidentdata2.mat'));
    fig2 = findall(0,'type','figure','tag','nnidentdata'); delete(fig2);
  ...
  drawnow;   % Replacing pause(1) with drawnow
    ...
    if (a1 < 1) | (a1 > 13), 
       trainfun=get_param(arg1,'trainfun'); 
       a1=nnstring.first_match(trainfun,func_index);
...
%   else
    ...
    if strcmp(parent_function,'narma_l2')
      ...
      if Use_Previous_Weights & ~isempty(nnstring.vert_cat(get_param(arg1,'IW1_1')))
        ...
        IW1_1a=eval(nnstring.vert_cat(get_param(arg1,'IW1_1')));
        IW3_2a=eval(nnstring.vert_cat(get_param(arg1,'IW3_2')));
        IW5_3a=eval(nnstring.vert_cat(get_param(arg1,'IW5_3')));
        LW2_1a=eval(nnstring.vert_cat(get_param(arg1,'LW2_1')));
        LW4_3a=eval(nnstring.vert_cat(get_param(arg1,'LW4_3')));
        LW5_4a=eval(nnstring.vert_cat(get_param(arg1,'LW5_4')));
        LW6_5a=eval(nnstring.vert_cat(get_param(arg1,'LW6_5')));
        LW6_2a=eval(nnstring.vert_cat(get_param(arg1,'LW6_2')));
        B1a=eval(nnstring.vert_cat(get_param(arg1,'B1')));
        B2a=eval(nnstring.vert_cat(get_param(arg1,'B2')));
        B3a=eval(nnstring.vert_cat(get_param(arg1,'B3')));
        B4a=eval(nnstring.vert_cat(get_param(arg1,'B4')));
      ...
      if Use_Previous_Weights & ~isempty(nnstring.vert_cat(get_param(arg1,'IW')))
        ...
        IW_2=eval(nnstring.vert_cat(get_param(arg1,'IW')));
        LW2_1_2=eval(nnstring.vert_cat(get_param(arg1,'LW2_1')));
        LW1_2_2=eval(nnstring.vert_cat(get_param(arg1,'LW1_2')));
        B1_2=eval(nnstring.vert_cat(get_param(arg1,'B1')));
        B2_2=eval(nnstring.vert_cat(get_param(arg1,'B2')));
  ...
  set(H.Simulating_text,'visible','off'); clear fig H
  save(cat(2,tempdir,'nnidentdata.mat'));
  fig = findall(0,'type','figure','tag','nnident'); H = get(fig,'userdata');
  parent_function=get(H.parent_function_ptr,'userdata');
  ...
  if strcmp(parent_function,'narma_l2')
    if Normalize
      ...
      set(f2.h2,'ylim',([min([ttr{1} Ysim{1} -1]) max([ttr{1} Ysim{1} 1])]+1)*abs(maxt-mint)/2+mint);
      ...
      set(f2.h4,'ylim',([min([ttr{1} Ysim{1} -1]) max([ttr{1} Ysim{1} 1])]+1)*abs(maxt-mint)/2+mint);
    else
    ...
    if Normalize
      ...
      set(f2.h2,'ylim',([min([ttr Ysim -1]) max([ttr Ysim 1])]+1)*abs(maxt-mint)/2+mint);
      plot((0:size(ptr,2)-1)*Ts,(ttr-Ysim)*(maxt-mint)/2,'Parent',f2.h3);
      plot((0:size(ptr,2)-1)*Ts,(Ysim+1)*(maxt-mint)/2+mint,'Parent',f2.h4);
      set(f2.h4,'ylim',([min([ttr Ysim -1]) max([ttr Ysim 1])]+1)*abs(maxt-mint)/2+mint);
    else
...
if Use_Validation
  ...
  if strcmp(parent_function,'narma_l2')
    if Normalize
      ...
      set(f2.h2,'ylim',([min([vv.T{1} Yvsim{1} -1]) max([vv.T{1} Yvsim{1} 1])]+1)*abs(maxt-mint)/2+mint);
      ...
      set(f2.h4,'ylim',([min([vv.T{1} Yvsim{1} -1]) max([vv.T{1} Yvsim{1} 1])]+1)*abs(maxt-mint)/2+mint);
    else
    ...
    if Normalize
      ...
      set(f2.h2,'ylim',([min([vv.T Yvsim -1]) max([vv.T Yvsim 1])]+1)*abs(maxt-mint)/2+mint);
      plot((0:size(vv.P,2)-1)*Ts,(vv.T-Yvsim)*(maxt-mint)/2,'Parent',f2.h3);
      plot((0:size(vv.P,2)-1)*Ts,(Yvsim+1)*(maxt-mint)/2+mint,'Parent',f2.h4);
      set(f2.h4,'ylim',([min([vv.T Yvsim -1]) max([vv.T Yvsim 1])]+1)*abs(maxt-mint)/2+mint);
    else
...
if Use_Testing
  ...
  if strcmp(parent_function,'narma_l2')
    if Normalize
      ...
      set(f2.h2,'ylim',([min([tt.T{1} Ytsim{1} -1]) max([tt.T{1} Ytsim{1} 1])]+1)*abs(maxt-mint)/2+mint);
      ...
      set(f2.h4,'ylim',([min([tt.T{1} Ytsim{1} -1]) max([tt.T{1} Ytsim{1} 1])]+1)*abs(maxt-mint)/2+mint);
    else
    ...
    if Normalize
      ...
      set(f2.h2,'ylim',([min([tt.T Ytsim -1]) max([tt.T Ytsim 1])]+1)*abs(maxt-mint)/2+mint);
      plot((0:size(tt.P,2)-1)*Ts,(tt.T-Ytsim)*(maxt-mint)/2,'Parent',f2.h3);
      plot((0:size(tt.P,2)-1)*Ts,(Ytsim+1)*(maxt-mint)/2+mint,'Parent',f2.h4);
      set(f2.h4,'ylim',([min([tt.T Ytsim -1]) max([tt.T Ytsim 1])]+1)*abs(maxt-mint)/2+mint);
    else
  ...
  if strcmp(parent_function,'narma_l2')
    ...
    netn2.IW{1}=iw{1}(:,Nj+1:Ni+Nj);
    netn2.LW{1,2}=iw{1}(:,1:Nj);
    netn2.layerweights{1,2}.learnFcn='learngdm';
...
function present_error(fig,H,text_field,field_value,field_type,theMessage)
...
set(H.error_messages,'string',theMessage);
errordlg(theMessage,'Plant Identification Warning','modal');
set(fig,'pointer','arrow');
...
try
    ...
    theMessage = 'Illegal value assigned to parameter';

    if ~sanitycheckparam(paramval, checkforinf)
        nnerr.throw('Parameters',theMessage);
    end
    
catch
    theMessage = sprintf('Illegal value assigned to ''%s'' parameter', paramlabel);
    errordlg(theMessage,'Plant Identification Warning','modal');
    paramok = false;
end
...
sunits = get(0, 'Units');
set (0, 'Units', 'character'); set (0, 'Units', 'points');
ssinchar = get(0, 'ScreenSize'); ssinchar = ssinchar ./ [1 1 3.75 9.75];
set (0, 'Units', sunits);
...
function x = convertEmptyBrackets(x)
