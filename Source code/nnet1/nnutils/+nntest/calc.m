...
%
%   ok = nntest.calc(seed)
%   ok = nntest.calc(net,x,xi,ai,t,ew,masks,seed)
%
...
if ~ok
  disp('--')
  disp(' ')
  disp('ATTEMPTING TO IDENTIFY DISCONTINUITIES IN INDIVIDUAL BLOCKS')
  discont = nnet.test.discontinuities(net,data.X,data.Xi,data.Ai);
  if ~isempty(discont)
    disp(' ');
    disp('*** SKIPPING PROBLEM ***')
    disp('*** KNOWN DISCONTINUITY ***')
    disp(' ')
    for i=1:numel(discont), disp(discont{i}), end
    ok = true;
  end
  return,
end
