function [out1,out2] = network_init_fcn(in1,in2,in3)
%NN_NETWORK_INIT_FCN Network initialization function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Network Initialization Function',7,...
    7,fullfile('nnet','nninitnetwork'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.network_init_fcn')
    err = [upper(x) '(''type'') is not nntype.network_init_fcn.'];
    return;
  end
