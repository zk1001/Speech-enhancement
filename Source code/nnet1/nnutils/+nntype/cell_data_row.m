function [out1,out2] = cell_data_row(in1,in2,in3)
%NN_CELL_DATA Neural network data in single row cell array form.
...
function err = type_check(x)
  ...
  if ~size(x,1)
    err = 'VALUE is not a row.';
  end
end

function x = strict_format(x)
  x = nntype.cell_data('format',x);
end
