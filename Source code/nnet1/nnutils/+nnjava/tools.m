...
if isempty(JAVA_TOOLS)
  ...
  JAVA_TOOLS.initialize(matlabroot,nnpath.nnet_toolbox,nnDependency.simulinkAvailable);
end
...
try
  switch command
    ...
    case 'get_java_net_input_function'
      nifname = nnjava.tools('string',varargin{1});
      result = JAVA_TOOLS.getNetInputFunction(nifname);
  ...
  %% CALLS FROM MATLAB
    ...
    case 'diagram'
      ...
      if isempty(net.name)
        diagram.name.set('Neural Network');
      else
      ...
      for i=1:net.numLayers
        ...
        for j=1:net.numLayers
          ...
          jNetInputFunction = nnjava.tools('get_java_net_input_function',net.layers{i}.netInputFcn);
          layers{i}.layerProperties.netInputFunction.set(jNetInputFunction);
    ...
    case 'view'
      ...
      drawnow
      nngui.publish_java_window(netview);
...
for i=1:length(variables)
  ...
  if ~strcmp(name,'ans')
    ...
    if ~isa(value,'gpuArray') && nntype.data('isa',value) && (~iscell(value) || (nnfast.numsignals(value)==1))
      if iscell(value)
        standardType = nn_type_category([value{:}]);
        transposedType = nn_type_category(vertcat(value{:})');
        if isempty(value)
          dimensions = [size(value,2) 0 0];
        else
    ...
    elseif isa(value,'network')
      standardType = '.NETWORK';
      transposedType = standardType;
      dimensions = [1 1 1];
      isCell = nnjava.tools('false');
      numFinite = 0;
    else
