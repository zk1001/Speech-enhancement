...
if isreal(a) && isreal(t)
  if any(size(a) ~= size(t)), error(message('nnet:postreg:ATdiffDim')), end
  if (show), clf reset, end
  [tsort,line,m,b,r] = postreg1(a,t);
  if(show), plotreg1(a,t,tsort,line,m,b,r,'','r'); end
elseif iscell(a) && iscell(t)
  if any(size(a) ~= size(t)), error(message('nnet:postreg:ATdiffDim')), end
  if size(a,1) ~= 1, error(message('nnet:NNet:ATNotRow')),end
  n = size(a,2);
  if (n<1) || (n>3), error(message('nnet:NNet:ATNot123El')),end
  for i=1:n
    if any(size(a{i})~=size(t{i})),error(message('nnet:NNet:AiTiMismatch')),end
  end
...
%set(gca,'xlim',[0 1])
%set(gca,'ylim',[0 1])
legend([name ' Data Points'],'Best Linear Fit','Y = T','Location','NorthWest');
hold off
