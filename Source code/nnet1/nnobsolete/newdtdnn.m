function out1 = newdtdnn(varargin)
...
info.name = 'Distributed Time Delay';
...
if isa(d,'cell')
  if (size(d,1) > 1)
    error(message('nnet:NNData:DelayNotRowVecOrRowCellVec'))
  end
  for i=1:length(d)
    id = d{i};
  ...
  error(message('nnet:NNData:DelayNotCell'))
end
...
if (~isa(pr,'double')) | ~isreal(pr) | (size(pr,2) ~= 2)
  error(message('nnet:NNData:RangeNot2ColMatOrCellOf'))
end
if any(pr(:,1) > pr(:,2))
