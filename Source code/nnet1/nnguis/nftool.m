...
try
  ...
  switch command
    ...
    case 'generateFunction'
      code = genFunction(STATE.net2,'myNeuralNetworkFunction','SampleColumn',STATE.sampleByColumn);
      code = nnet.text.text2str(code);
      nnet.genscript.sendToEditor(code);
      
    case 'generateMatrixFunction'
      code = genFunction(STATE.net2,'myNeuralNetworkFunction','MatrixOnly','yes','SampleColumn',STATE.sampleByColumn);
      code = nnet.text.text2str(code);
      nnet.genscript.sendToEditor(code);
      
    case 'generateTrainScriptSimple'
      code = generateSimpleScript(nnet.genscript.FittingScriptGenerator(STATE));
      code = nnet.text.text2str(code);
      nnet.genscript.sendToEditor(code);
      
    case 'generateTrainScriptAdvanced'
      code = generateAdvancedScript(nnet.genscript.FittingScriptGenerator(STATE));
      code = nnet.text.text2str(code);
      nnet.genscript.sendToEditor(code);
...
try
  % Try with selected training function, if memory requirements allow
  [net2,tr] = train(STATE.net1,x,t);
catch me
  % Try TRAINSCG if failed
