...
for i=ti
  ii = outputs2layers(i);
  net = nnet.internal.configure.output(net,ii,T{i});
end
...
% Throw error for Composite and NNDATA2GPU data
function d = iEnsureDataSuitability(d)
  if isa(d,'Composite')
    error(message('nnet:configure:CompositeDataUnconfigured'));
  elseif isa(d,'gpuArray') && nnet.array.isNNData2Gpu(d)
