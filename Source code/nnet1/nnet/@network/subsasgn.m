...
switch type
    case '.'
        ...
        switch(field)
            ...
            case 'input'
                if (net.numInputs ~= 1)
                    error( message( 'nnet:NNet:InputOnlyForSingleInput' ) );
                end
                fieldSub.type = '.';
            ...
            case 'output'
                if (net.numOutputs ~= 1)
                    error( message( 'nnet:NNet:OutputOnlyForSingleOutput' ) );
                end
                fieldSub.type = '.';
...
for i = find(oldBiasConnect ~= biasConnect)'
    if biasConnect(i)
        ...
        net = nnet.internal.configure.bias(net,i);
    else
        net.biases{i} = [];
        net.b{i} = [];
    end
...
for i=1:net.numLayers
    for j = find(oldInputConnect(i,:) ~= inputConnect(i,:))
        if inputConnect(i,j)
            ...
            if isempty(x)
                net = nnet.internal.configure.inputWeight(net,i,j);
            else
                % NNET 6.0 Compatibility
                net = nnet.internal.configure.inputWeight(net,i,j,x);
            end
        else
...
for i=1:net.numLayers
    for j = find(oldLayerConnect(i,:) ~= layerConnect(i,:))
        if layerConnect(i,j)
            ...
            net = nnet.internal.configure.layerWeight(net,i,j);
        else
...
for i = find(oldOutputConnect ~= outputConnect)
    if outputConnect(i)
        ...
        net = nnet.internal.configure.output(net,i,net.layers{i}.range);
    else
        net.outputs{i} = [];
    end
end
...
net = nnet.internal.configure.input(net,j,exampleInput);

% Feedback
fbindex = net.inputs{j}.feedbackOutput;
if ~isempty(fbindex)
    net.outputs{fbindex}.exampleOutput = exampleInput;
    net = nnet.internal.configure.output(net,fbindex,exampleInput);
end
...
net = nnet.internal.configure.input(net,j,net.inputs{j}.exampleInput);
...
if ~isempty(fbindex)
    ...
    net = nnet.internal.configure.output(net,fbindex,net.inputs{j}.exampleInput);
end
...
% Changes
net = nnet.internal.configure.input(net,j,range);

% Feedback
fbindex = net.inputs{j}.feedbackOutput;
if ~isempty(fbindex)
    net = nnet.internal.configure.output(net,fbindex,range);
end
...
% Changes
net = nnet.internal.configure.layer(net,i,newDimensions);
...
if sizeChange && net.outputConnect(i)
    ...
    if ~isempty(fbindex)
        ...
        net = nnet.internal.configure.input(net,fbindex,x);
    end
end
...
net = nnet.internal.configure.output(net,j,exampleOutput);

% Feedback
fbindex = net.outputs{j}.feedbackInput;
if ~isempty(fbindex)
    net.inputs{fbindex}.exampleInput = exampleOutput;
    net = nnet.internal.configure.input(net,fbindex,exampleOutput);
end
...
net = nnet.internal.configure.output(net,j,net.outputs{j}.exampleOutput);
...
if ~isempty(fbindex)
    ...
    net = nnet.internal.configure.input(net,fbindex,net.outputs{j}.exampleOutput);
end
...
net = nnet.internal.configure.output(net,j,range);
