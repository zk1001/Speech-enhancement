...
for i=1:numSubscripts
  ...
  case '.'
    ...
    if strcmpi(subs,'input')
      if (v.numInputs ~= 1)
        nnerr.throw('Args','Property ''input'' is only valid for networks with exactly one input.')
      else
        v = v.inputs{1};
    ...
    if strcmpi(subs,'output')
      if (v.numOutputs ~= 1)
        nnerr.throw('Args','Property ''input'' is only valid for networks with exactly one output.')
      else
