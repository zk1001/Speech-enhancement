...
if nargout == 2
  [Y,Af] = nnMex.yy(net,data.X,data.Xi,data.Pc,data.Pd,data.Ai,data.Q,data.TS,hints,TEMP,hints.batch);
else
  Y = nnMex.yy(net,data.X,data.Xi,data.Pc,data.Pd,data.Ai,data.Q,data.TS,hints,TEMP,hints.batch);
end
