...
% targets.
%
% <a href="matlab:doc fixunknowns">fixunknowns</a> processes data by replacing each row containing unknown
% values (represented by NaN) with two rows. The first row contains the
% original row, with NaN values replaced by the row's mean. The second row
% contains 1 and 0 values, indicating which values in the first row were
% known or unknown, respectively. Using FIXUNKNOWNS as an input processing
% function allows a network to use data with unknowns, and even perhaps use
% the existence of unknowns as useful information.
%
% [Y,settings] = <a href="matlab:doc fixunknowns">fixunknowns</a>(X) takes a matrix of input data, transforms 
% it, and returns the result and the settings used to perform the 
% transform.
%
