...
ind = iGetIndicesForEachConfusionMatrixEntry(i, j, numClasses);
...
function ind = iGetIndicesForEachConfusionMatrixEntry(targets, outputs, numClasses)
ind = cell(numClasses);
flattenedConfusionIndices = sub2ind([numClasses numClasses], targets, outputs);
for k = 1:numClasses*numClasses
    ind{k} = find(flattenedConfusionIndices == k);
end
end