...
if isempty(get(fig2,'children')), fig2 = 0; end

% Get name from SIMULINK block if available
if (~isempty(ver('simulink'))) && (gcbh ~= -1)
...
if (epoch==0) || newplot
   ...
   ud.TrainLine1    = plot(0,0,0,0,0,0,'Parent',ud.TrainAxes1);
   ...
   ud.TrainLine2    = plot(0,0,'Parent',ud.TrainAxes2);
   ...
   ud.TrainLine3    = plot(0,0,'Parent',ud.TrainAxes3);
...
if epoch~=0
   % Changes to avoid error when min_gamk == max_gamk
...
if epoch~=0
   % Change to avoir error when min_trperf or max_trperf are NaN
