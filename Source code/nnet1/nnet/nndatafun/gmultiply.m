function c = gmultiply(a,b)
%GMULTIPLY Generalized multiplication..
%
% <a href="matlab:doc gmultiply">gmultiply</a>(a,b) returns a .* b, supporting built in data behavior, as well as
% generalized behavior such as element-by-element and recursive multiplication
...
%
% Here are examples of multiplication with generalized behavior:
%
%   <a href="matlab:doc gmultiply">gmultiply</a>([1 2 3; 4 5 6],[10;20])
%   <a href="matlab:doc gmultiply">gmultiply</a>({1 2; 3 4},{1 3; 5 2})
%   <a href="matlab:doc gmultiply">gmultiply</a>({1 2 3 4},{10;20;30})
%
% See also GADD, GSUBTRACT, GDIVIDE, GNEGATE.
...
elseif numel(a) == 1
  a = a{1};
  c = cell(size(b));
  for i=1:numel(b), c{i} = calc_general(a,b{i}); end
  return;
end
...
if (adims==bdims) && all(asize==bsize)
  c = cell(asize);
  for i=1:numel(a), c{i} = calc_general(a{i},b{i}); end;
  return
end
