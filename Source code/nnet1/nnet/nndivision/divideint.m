...
%
% <a href="matlab:doc divideint">divideint</a> starts with index 1, alternately assigns each index to
% whichever one of the three sets as will help best match the desired
% ratios until all Q indices are assigned.
%
% For example, here 250 samples are divided into 70% for training, 15%
% for validation and 15% for testing.
%
%   [trainInd,valInd,testInd] = <a href="matlab:doc divideint">divideint</a>(250,0.7,0.15,0.15)
%
% Here is how to ensure a network will perform the same kind of data
% division when it is trained:
%
%   net.<a href="matlab:doc nnproperty.net_divideFcn">divideFcn</a> = '<a href="matlab:doc divideint">divideint</a>';
...
%
% See also divideblock, divideind, dividerand, dividetrain.
...
function info = get_info
  info = nnfcnDivision(mfilename,'Interleaved',fcnversion, ...
    [ ...
...
function [trainInd,valInd,testInd] = divide_indices(Q,params)
  ...
  trainPercent = params.trainRatio/totalRatio;
  testPercent = params.testRatio/totalRatio;
  valPercent = params.valRatio/totalRatio;
  percents = [trainPercent valPercent testPercent];
  totals = [0 0 0];
  slots = zeros(1,Q);
  for i=1:Q
    if i==1
